...
const Worksheet = (function() {
    ...
    function renderWorksheet() {
        ...
        for (let row = 0; row < totalRows; row++) {
            ...
            // 创建行容器（实际包含内容的 grid 项）
            const rowContainer = document.createElement("div");
            rowContainer.className = "worksheet-row";

            // 在每行之前插入信息展示区域
            const infoRow = createInfoRow(charData);
            rowContainer.appendChild(infoRow);
    ...
    // =========================================================================
    // Print Pagination - Pre-calculate page breaks
    // =========================================================================

    const PAGE_CONFIG = {
        // A4 纸张尺寸（毫米）- 使用实际可打印高度
        a4: { width: 210, height: 290 },
        // 打印边距（最小边距，最大化可打印空间）
        margin: { top: 0, right: 0, bottom: 0, left: 0 }
    };

    /**
     * Calculate printable area based on orientation
     * @param {string} orientation - 'landscape' or 'portrait'
     * @returns {Object} Printable dimensions in mm
     */
    function getPrintableArea(orientation) {
        const isLandscape = orientation === "landscape";
        const pageWidth = isLandscape ? PAGE_CONFIG.a4.height : PAGE_CONFIG.a4.width;
        const pageHeight = isLandscape ? PAGE_CONFIG.a4.width : PAGE_CONFIG.a4.height;

        return {
            width: pageWidth - PAGE_CONFIG.margin.left - PAGE_CONFIG.margin.right,
            height: pageHeight - PAGE_CONFIG.margin.top - PAGE_CONFIG.margin.bottom
        };
    }

    /**
     * Measure actual row height in pixels
     * @returns {number} Row height in pixels
     */
    function measureRowHeight() {
        const container = elements.worksheetContainer;
        const firstRowWrapper = container.querySelector(".worksheet-row-wrapper");

        if (firstRowWrapper) {
            // Get the actual rendered height of the wrapper
            const rect = firstRowWrapper.getBoundingClientRect();
            return rect.height > 0 ? rect.height : 130; // Default fallback
        }

        return 130; // Default estimate
    }

    /**
     * Calculate how many rows fit per page
     * @param {number} rowHeightPx - Row height in pixels
     * @returns {number} Number of rows per page
     */
    function calculateRowsPerPage(rowHeightPx) {
        const printableArea = getPrintableArea(state.printOrientation);

        // Convert mm to pixels (standard 96 DPI)
        const theoreticalHeightPx = (printableArea.height / 25.4) * 96;

        // 浏览器打印时有少量页眉页脚占用空间，使用较小值
        // 减去约 5mm 浏览器默认占用
        const browserOverheadPx = (5 / 25.4) * 96; // ~19px
        const safeHeight = theoreticalHeightPx - browserOverheadPx;

        // Calculate rows per page (floor to ensure fit)
        const rowsPerPage = Math.floor(safeHeight / rowHeightPx);

        console.log(`[Print] Theoretical: ${theoreticalHeightPx.toFixed(0)}px, Safe: ${safeHeight.toFixed(0)}px, Row: ${rowHeightPx.toFixed(0)}px, Rows: ${rowsPerPage}`);

        // Minimum 3 rows per page
        return Math.max(rowsPerPage, 3);
    }

    /**
     * Insert page breaks before printing
     */
    function insertPageBreaks() {
        const container = elements.worksheetContainer;
        const rowWrappers = container.querySelectorAll(".worksheet-row-wrapper");
        ...
        console.log(`[Print] Row height: ${rowHeight}px, Rows per page: ${rowsPerPage}`);

        // Insert page breaks at calculated positions
        rowWrappers.forEach((wrapper, index) => {
            // Insert before each new page (except first)
            if (index > 0 && index % rowsPerPage === 0) {
                const pageBreak = document.createElement("div");
                pageBreak.className = "page-break";
                pageBreak.setAttribute("data-print-break", "true");
                // Insert before the row wrapper
                container.insertBefore(pageBreak, wrapper);
            }
        });
    }

    /**
     * Clear all temporary page breaks
     */
    function clearPageBreaks() {
        document.querySelectorAll(".page-break[data-print-break='true']").forEach(el => el.remove());
    }
